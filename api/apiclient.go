...
type Client interface {
	...
	AttachMetadataToObject(object_id int64, body map[string]interface{}) (*[]Metadata, error)
	DetachMetadataFromObject(object_id int64) (*[]Metadata, error)
	...
	GetExportByFileSystem(filesystem_id int64) (*[]ExportResponse, error)
	AddNodeInExport(export_id int, access string, noRootSquash bool, ip string) (*ExportResponse, error)
	DeleteNodeFromExport(export_id int, access string, noRootSquash bool, ip string) (*ExportResponse, error)
}
...
func (c *ClientService) GetNetworkSpaceByName(networkSpaceName string) (nspace NetworkSpace, err error) {
	...
	spacesUri := "api/rest/network/spaces"
	queryParam := map[string]interface{}{"name": networkSpaceName}
	resp, err := c.getResponseWithQueryString(spacesUri, queryParam, &netspaces)
	if err != nil {
...
func (c *ClientService) getJSONResponse(method, apiuri string, body, expectedResp interface{}) (resp interface{}, err error) {
	...
	if method == http.MethodPost {
		resp, err = c.api.Post(context.Background(), apiuri, nil, body, expectedResp)
	} else if method == http.MethodGet {
		resp, err = c.api.Get(context.Background(), apiuri, nil, expectedResp)
	} else if method == http.MethodDelete {
		resp, err = c.api.Delete(context.Background(), apiuri, nil)
	} else if method == http.MethodPut {
		resp, err = c.api.Put(context.Background(), apiuri, nil, body, expectedResp)
	}
	if err != nil {
...
func (c *ClientService) getResponseWithQueryString(apiuri string, queryParam map[string]interface{}, expectedResp interface{}) (resp interface{}, err error) {
	...
	resp, err = c.api.GetWithQueryString(context.Background(), apiuri, queryString, expectedResp)
	//expectedResp = resp
