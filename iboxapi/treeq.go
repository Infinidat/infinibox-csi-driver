...
func (client *IboxClient) GetTreeqByName(fsID int, name string) (treeq *Treeq, err error) {
	...
	if len(response.Result) == 0 {
		return nil, &APIError{Code: RESOURCE_NOT_FOUND, Err: fmt.Errorf("%s - treeq %s not found", functionName, name)}
	}
...
func (client *IboxClient) GetTreeq(fsID, treeqID int) (treeq *Treeq, err error) {
	...
	if response.Error.Code != "" {
		if response.Error.Code == TREEQ_ID_DOES_NOT_EXIST {
			return nil, &APIError{Code: RESOURCE_NOT_FOUND, Err: fmt.Errorf("%s - fs %d treeq %d not found", functionName, fsID, treeqID)}
		}
		return nil, fmt.Errorf("%s - ibox API - error: %v", functionName, response.Error)
	}
...
func (client *IboxClient) DeleteTreeq(fsID, treeqID int) (treeq *Treeq, err error) {
	...
	if response.Error.Code != "" {
		if response.Error.Code == TREEQ_ID_DOES_NOT_EXIST {
			return nil, &APIError{Code: RESOURCE_NOT_FOUND, Err: fmt.Errorf("%s - fs ID %d treeq ID %d not found", functionName, fsID, treeqID)}
		}
		return nil, fmt.Errorf("%s - ibox API - error: %v", functionName, response.Error)
	}
...
func (client *IboxClient) UpdateTreeq(fsID, treeqID int, updateRequest UpdateTreeqRequest) (*Treeq, error) {
	...
	if resp.Error.Code != "" {
		...
		if resp.Error.Code == TREEQ_ID_DOES_NOT_EXIST {
			return nil, &APIError{Code: RESOURCE_NOT_FOUND, Err: fmt.Errorf("%s- fs ID %d treeq ID '%d' does not exist", functionName, fsID, treeqID)}
		}
		return nil, fmt.Errorf("%s - ibox API - error: %v", functionName, resp.Error)
	}
