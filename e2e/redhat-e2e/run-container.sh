...
docker run  $OPTIONAL_DOCKER_BUILD_FLAGS -v $WORKDIR:/data:z --rm -it registry.redhat.io/openshift4/ose-tests:$VERSION sh -c "KUBECONFIG=/data/kubeconfig.yaml TEST_CSI_DRIVER_FILES=/data/e2e-manifest.yaml /usr/bin/openshift-tests run openshift/csi --junit-dir /data/results"
