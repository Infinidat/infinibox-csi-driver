...
func (s *service) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	...
	log.Infof("NodePublishVolume volproto", volproto)
	if len(volproto) != 2 {
	...
	// get operator
	storageNode, err := storage.NewStorageNode(volproto[1], config)
	if storageNode != nil {
...
func (s *service) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	...
	protocolOperation, err := storage.NewStorageNode(volproto[1])
	if err != nil {
		return nil, status.Error(codes.Internal, err.Error())
	}
	return protocolOperation.NodeUnpublishVolume(ctx, req)
}
