...
func (cas *CSIAddonsServer) serve(listener net.Listener) {
	slog.Debug("listening for CSI-Addons requests", "address", listener.Addr())

	// start to serve requests
	err := cas.server.Serve(listener)
	if err != nil && !errors.Is(err, grpc.ErrServerStopped) {
		slog.Error("failed to setup CSI-Addons server", "error", err)
	}

	slog.Debug("the CSI-Addons server has been stopped", "address", listener.Addr())
}
