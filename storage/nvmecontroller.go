...
func (nvme *nvmestorage) ControllerPublishVolume(ctx context.Context, req *csi.ControllerPublishVolumeRequest) (resp *csi.ControllerPublishVolumeResponse, err error) {
	...
	v, err := nvme.cs.Api.GetVolume(volproto.VolumeID)
	if err != nil {
...
func (nvme *nvmestorage) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (resp *csi.ControllerUnpublishVolumeResponse, err error) {
	...
	if len(host.Luns) > 0 {
		...
		err = nvme.cs.unmapVolumeFromHost(host.ID, volproto.VolumeID)
		if err != nil {
			e := fmt.Errorf("failed to unmap volume with ID %d from host with ID %d. Error: %v", volproto.VolumeID, host.ID, err)
			zlog.Err(e)
			return nil, status.Error(codes.Internal, e.Error())
		}
	}
	...
	if len(luns) == 0 {
		...
		if createdByCSI {
			...
			zlog.Debug().Msgf("ControllerUnpublishVolume deleted host on ibox because it was created by CSI host %d %s", host.ID, host.Name)
		} else {
