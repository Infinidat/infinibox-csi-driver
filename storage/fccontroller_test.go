...
func (suite *FCControllerSuite) Test_DeleteVolume_DeleteVolume_success() {
	...
	_, err := service.DeleteVolume(context.Background(), crtValReq)
	assert.Nil(suite.T(), err, "delete success for fc protocol")
}
...
func (suite *FCControllerSuite) Test_ControllerPublishVolume() {
	service := fcstorage{cs: *suite.cs}
//	var parameterMap map[string]string
	...
	assert.Nil(suite.T(), err, "fail to control publish for fc protocol")
}
