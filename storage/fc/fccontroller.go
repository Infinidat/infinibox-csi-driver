...
func (fc *FCstorage) CreateVolume(ctx context.Context, req *csi.CreateVolumeRequest) (*csi.CreateVolumeResponse, error) {
	...
	if targetVol != nil {
		...
		err = status.Errorf(codes.AlreadyExists, "volume: %s already exists with a different size, %v", name, err)
	...
	if err != nil {
		e := fmt.Sprintf("error from DetermineSSDValue volumeName: %s storagePool: %s, error: %s", name, poolName, err.Error())
		slog.Error(e)
	...
	if err != nil {
		e := fmt.Sprintf("error from GetPoolByName volumeName: %s poolName: %s error: %s", name, poolName, err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
	...
	if err != nil {
		e := fmt.Sprintf("error from CreateVolume volumeName: %s storagepool: %s, error: %s", name, poolName, err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
	...
	if err != nil {
		slog.Error("error from GetVolume", "error", err.Error())
	}
	...
	if vol == nil {
		...
		if err != nil {
			e := fmt.Sprintf("error from GetVolume - volumeName: %s volumeID: %d, error: %s", name, volumeResp.ID, err.Error())
			slog.Error(e)
			return nil, status.Error(codes.Internal, e)
		}
	}
	...
	if err != nil {
		e := fmt.Sprintf("error from PutMetadata - volumeName: %s error: %s", name, err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
...
func (fc *FCstorage) ControllerPublishVolume(ctx context.Context, req *csi.ControllerPublishVolumeRequest) (resp *csi.ControllerPublishVolumeResponse, err error) {
	...
	if err != nil {
		e := fmt.Sprintf("error from ValidateVolumeID - error: %s", err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
	...
	if err != nil {
		e := fmt.Sprintf("error from DetermineHostName - error: %s", err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
	...
	if err != nil {
		e := fmt.Sprintf("error from ValidateHost hostName: %s  error: %s", hostName, err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
	...
	if err != nil {
		e := fmt.Sprintf("error from GetVolume volumeID: %s error: %s", req.GetVolumeId(), err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
	...
	if err != nil {
		e := fmt.Sprintf("error from IsValidAccessMode - error: %s", err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
	...
	if err != nil {
		e := fmt.Sprintf("error from GetAllLunByHost volumeName: %s hostID: %d  error: %s", volume.Name, host.ID, err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
	var ports string
	...
	if maxAllowedVolString != "" {
		...
		if err != nil {
			e := fmt.Sprintf("invalid parameter: %s error: %s", common.StorageClassMaxVolsPerHost, err.Error())
			slog.Error(e)
			return nil, status.Error(codes.Internal, e)
		}
		if maxAllowedVol < 1 {
			e := fmt.Sprintf("invalid parameter: %s error: required to be greater than 0", common.StorageClassMaxVolsPerHost)
			slog.Error(e)
			return nil, status.Error(codes.Internal, e)
		}
		...
		if len(lunList) >= maxAllowedVol {
			e := fmt.Sprintf("unable to publish volume on hostName: %s, maximum allowed volume per host: %d, limit reached", host.Name, maxAllowedVol)
			slog.Error(e)
	...
	if err != nil {
		e := fmt.Sprintf("error from MapVolumeToHost - error: %s", err.Error())
		slog.Error(e)
		return nil, status.Error(codes.Internal, e)
	}
...
func (fc *FCstorage) ControllerUnpublishVolume(ctx context.Context, req *csi.ControllerUnpublishVolumeRequest) (resp *csi.ControllerUnpublishVolumeResponse, err error) {
	slog.Debug("start", "volProto.VolumeID", fc.CS.VolProto.VolumeID, "nodeID", req.GetNodeId(), "volumeId", req.GetVolumeId())

	host := fc.CS.VolProto.Host
	if len(host.Luns) > 0 {
		slog.Debug("unmap volume", "ID", fc.CS.VolProto.VolumeID, "from host", host.ID)
		err = fc.CS.UnmapVolumeFromHost(ctx, host.ID, fc.CS.VolProto.VolumeID)
		if err != nil {
			e := fmt.Sprintf("error from UnmapVolumeFromHost - volumeID: %d hostID: %d error: %s", fc.CS.VolProto.VolumeID, host.ID, err.Error())
			slog.Error(e)
			return nil, status.Error(codes.Internal, e)
		}
	}
	if len(host.Luns) < 2 {
		...
		if err != nil {
			e := fmt.Sprintf("error from HostCleanup - hostID: %d error: %s", host.ID, err.Error())
			slog.Error(e)
			return nil, status.Error(codes.Internal, e)
		}
	}
...
func (fc *FCstorage) CreateSnapshot(ctx context.Context, req *csi.CreateSnapshotRequest) (resp *csi.CreateSnapshotResponse, err error) {
	...
	slog.Debug("start", "name", snapshotName, "source volume ID", req.GetSourceVolumeId(), "volproto", fc.CS.VolProto)
	...
	if err != nil {
		e := fmt.Sprintf("error from GetVolume - getting parent volume when creating snapshot - volumeID: %d, error: %s", fc.CS.VolProto.VolumeID, err.Error())
		slog.Error(e)
