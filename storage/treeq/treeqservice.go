...
func (ts *TreeqService) checkTreeqName(FileSystems []iboxapi.FileSystem, pVName string) (treeqData *iboxapi.Treeq) {
	type item struct {
		treeq *iboxapi.Treeq
		err   error
	}
...
func (ts *TreeqService) IsTreeqAlreadyExist(poolName, networkSpace, pVName, fsPrefix string) (treeqVolumeContext map[string]string, err error) {
	...
	fsMetaData, poolErr := ts.CS.IboxApi.GetFileSystemsByPool(ts.PoolID, fsPrefix)
	if poolErr != nil {
	...
	treeqData := ts.checkTreeqName(fsMetaData, pVName)
	if treeqData != nil {
...
func (ts *TreeqService) getExpectedFileSystemID(maxFileSystemSize int64) (filesys *iboxapi.FileSystem, err error) {
	...
	for _, fs := range fsMetaData {
		if fs.Size+ts.NFSstorage.Capacity < maxFileSystemSize {
			treeqs, treeqCnterr := ts.CS.IboxApi.GetTreeqsByFileSystem(fs.ID)
			if treeqCnterr != nil {
...
func (svc *TreeqService) UpdateTreeqVolume(filesystemID, treeqID int, capacity int64, maxFileSystemSize string) (err error) {
	...
	for _, t := range treeqsInFileSystem {
		totalTreeqSize = totalTreeqSize + t.HardCapacity
	}
