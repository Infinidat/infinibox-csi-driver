...
func (treeq *Treeqstorage) NodePublishVolume(ctx context.Context, req *csi.NodePublishVolumeRequest) (*csi.NodePublishVolumeResponse, error) {
	slog.Debug("NodePublishVolume (treeq)", "iboxInfo", storagecommon.GetHostInfo(ctx, req.GetSecrets(), treeq.NFSstorage.CS.IboxAPI))
	...
	slog.Debug("NodePublishVolume (treeq)", "targetPath", hostTargetPath, "volume id", req.GetVolumeId())
	...
	slog.Debug("NodePublishVolume (treeq)", "filesystem id", fileSystemID, "treeq id", treeqID, "volume context", req.GetVolumeContext(), "storageclass params", treeq.NFSstorage.StorageClassParameters)
	...
	slog.Debug("NodePublishVolume (treeq)", "exports", len(exports), "file system id", fileSystemID)

	if len(exports) == 0 {
		exportAccess := "RW"
		if req.GetReadonly() || req.VolumeCapability.GetAccessMode().GetMode() == csi.VolumeCapability_AccessMode_MULTI_NODE_READER_ONLY {
			slog.Debug("NodePublishVolume (treeq) - detected read-only, setting export to RO")
			exportAccess = "RO"
		}
		...
		if req.GetVolumeContext()[common.StorageClassNFSExportPermissions] != "" {
			...
			slog.Debug("NodePublishVolume (treeq) will not create default export rule, will create this rule instead", "sc export perms", common.StorageClassNFSExportPermissions, "export persm", exportPerms)
		}
		...
		slog.Debug("NodePublishVolume (treeq) - skipping updateExport because other exports exist")
	}

	_, err = os.Stat(hostTargetPath)
	if os.IsNotExist(err) {
		slog.Debug("NodePublishVolume (treeq) - targetPath does not exist, will create", "targetpath", targetPath)
		...
		if err != nil {
			slog.Error("NodePublishVolume (treeq) - host target path exists", "error", err.Error())
		}
		slog.Debug("NodePublishVolume (treeq) - targetPath already exists, will not do anything", "targetpath", targetPath)
	...
	if dnsName != "" {
		sourceIP = dnsName
		slog.Debug("NodePublishVolume (treeq) - storageclass has dnsname specified, using it for mount instead of ipAddress", "dnsname", dnsName)
	}
	...
	slog.Debug("NodePublishVolume (treeq) - GetNFSVersionPort", "version", nfsVersion, "port", nfsPort)
	...
	slog.Debug("NodePUblishVolume (treeq) mount", "sourcePath", source, "targetpath", targetPath)
	...
	slog.Debug("NodePublishVolume (treeq) - mounted treeq volume", "source", source, "volume id", req.GetVolumeId(), "targetPath", targetPath, "mountoptions", mountOptions)

	if req.GetReadonly() || req.VolumeCapability.GetAccessMode().GetMode() == csi.VolumeCapability_AccessMode_MULTI_NODE_READER_ONLY {
		slog.Debug("NodePublishVolume (treeq) - this is a readonly volume, skipping setting volume permissions")
...
func (treeq *Treeqstorage) NodeUnpublishVolume(ctx context.Context, req *csi.NodeUnpublishVolumeRequest) (*csi.NodeUnpublishVolumeResponse, error) {
	slog.Debug("NodeUnpublishVolume (treeq) starts")
